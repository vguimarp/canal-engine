...
import { currentUserId, resolveBodyChannel } from "@/lib/tenant";
...
export async function POST(request) {
  ...
  const trends = await ai.generateTrends(channel.niche, 10);
  ...
  const ideas = await ai.generateIdeas(channel.niche, topics, { longCount: 5, shortCount: 10 });
  ...
  // 3) SEO
  const kws = await ai.generateKeywords(topics.slice(0, 6));

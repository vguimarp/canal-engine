...
import { ensureSubscriptionForUser } from "./billing.js";
...
export function createUser({ email, name, password }) {
  ...
  const r = db.prepare("INSERT INTO users (email, name, password_hash, plan, status) VALUES (?,?,?,?,?)")
    .run(e, name || null, hashPassword(password), "free", "active");
  ...
  const ws = db.prepare("INSERT INTO workspaces (name, owner_user_id, owner_id, is_demo) VALUES (?,?,?,0)")
    .run(`${name || e} — Workspace`, userId, userId);
  ...
  ensureSubscriptionForUser(userId, ws.lastInsertRowid, "free");
  syncDb();
  return { id: userId, email: e, name: name || null, plan: "free", role: "user", workspaceId: ws.lastInsertRowid };
}

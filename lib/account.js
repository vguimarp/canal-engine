...
export function validateSignupInput(data = {}) {
  ...
  if (!String(data.name || "").trim()) errors.push("Nome completo é obrigatório.");
  ...
  if (phone.length < 10 || phone.length > 15) errors.push("Telefone celular inválido.");
  if (whatsapp.length < 10 || whatsapp.length > 15) errors.push("WhatsApp inválido.");
  if (!String(data.country || "").trim()) errors.push("País é obrigatório.");
  if (!String(data.state || "").trim()) errors.push("Estado é obrigatório.");
  if (!String(data.city || "").trim()) errors.push("Cidade é obrigatória.");
  if (!USER_TYPES.includes(String(data.userType || data.user_type || ""))) errors.push("Tipo de usuário inválido.");
  if (!data.termsAccepted) errors.push("Aceite os Termos de Uso.");
  if (!data.privacyAccepted) errors.push("Aceite a Política de Privacidade.");
...
export function createFullUser(data = {}, request = null) {
  ...
  const userType = String(data.userType || data.user_type);

...
// Só usamos Turso quando a URL E o token estão presentes. URL sem token é a
// causa clássica de "Unauthorized at new Database".
const USE_TURSO = !!TURSO_URL_RAW && !!TURSO_TOKEN;
...
export function getDb() {
  ...
  if (USE_TURSO) {
    ...
    if (TURSO_URL_RAW && !TURSO_TOKEN) {
      console.warn("[db] TURSO_DATABASE_URL definido SEM TURSO_AUTH_TOKEN — Turso ignorado, usando banco local.");
    }
    db = openDemo();
  }

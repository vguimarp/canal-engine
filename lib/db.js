...
// Auto-semeia quando o banco conecta vazio (Turso recém-criado ou demo /tmp).
// Garante que o sistema NUNCA mostre listas vazias e, no Turso, persiste os dados.
function autoSeedIfEmpty(db) {
  try {
    const n = db.prepare("SELECT COUNT(*) c FROM channels").get().c;
    if (n > 0) return;
    // Transação única: batela ~1500 inserts (rápido e atômico; evita duplicar
    // se dois cold starts correrem juntos — o 2º vê count>0 e sai).
    db.transaction(() => {
      if (db.prepare("SELECT COUNT(*) c FROM channels").get().c > 0) return;
      seedDatabase(db);
    })();
    console.log("[db] Banco vazio detectado — populado automaticamente com os 5 canais de demonstração.");
  } catch (e) {

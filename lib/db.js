...
function migrate(db) {
  ...
  for (const [column, decl] of [
    ...
    ["updated_at", "TEXT"],
    ["last_login_at", "TEXT"],
  ]) addColumn("users", column, decl);
  addColumn("channels", "workspace_id", "INTEGER");
  addColumn("channels", "owner_user_id", "INTEGER");
  addColumn("workspaces", "owner_id", "INTEGER");
  addColumn("media_assets", "file_name", "TEXT");
  addColumn("media_assets", "file_path", "TEXT");
  addColumn("media_assets", "asset_mime", "TEXT");
  addColumn("media_assets", "asset_content", "TEXT");
  ...
  try {
    const adminEmail = process.env.ADMIN_EMAIL || process.env.MASTER_ADMIN_EMAIL;
    if (adminEmail) db.prepare("UPDATE users SET role='admin' WHERE lower(email)=lower(?)").run(adminEmail);
    db.prepare("UPDATE users SET status='active' WHERE status IS NULL").run();
    db.prepare("UPDATE workspaces SET owner_id=owner_user_id WHERE owner_id IS NULL AND owner_user_id IS NOT NULL").run();
...
function seedPlans(db) {
  try {
    ...
    stmt.run("starter", "STARTER", 3, 100, 20, 1, 0);
    stmt.run("pro", "PRO", 10, null, null, 1, 0);
    stmt.run("agency", "AGENCY", null, null, null, null, 1);
    const price = db.prepare(`INSERT INTO billing_prices
      (plan_code, interval, provider, provider_price_id, amount_cents, currency, active)
      VALUES (?,?,?,?,?,?,1)
      ON CONFLICT(plan_code, interval, provider) DO UPDATE SET
        provider_price_id=excluded.provider_price_id,
        amount_cents=excluded.amount_cents,
        currency=excluded.currency,
        active=1`);
    const prices = [
      ["starter", "monthly", 1900], ["starter", "annual", 19000],
      ["pro", "monthly", 4900], ["pro", "annual", 49000],
      ["agency", "monthly", 14900], ["agency", "annual", 149000],
    ];
    for (const [plan, interval, amount] of prices) {
      price.run(plan, interval, "stripe", process.env[`STRIPE_${plan.toUpperCase()}_${interval.toUpperCase()}_PRICE_ID`] || null, amount, "BRL");
      price.run(plan, interval, "mercado_pago", process.env[`MP_${plan.toUpperCase()}_${interval.toUpperCase()}_PRICE_ID`] || null, amount, "BRL");
      price.run(plan, interval, "pix", null, amount, "BRL");
    }

...
export function getChannelsWithStats() {
  const db = getDb();
...
export function getDashboard(channelId = 1) {
  const db = getDb();
  ...
  return {
    ...
    analytics: getAnalyticsExecutive(channelId),
...
export function getExecutiveOverview() {
  const channels = getChannelsWithStats(); // já ordenado por monetizationPotential desc
  const byNiche = {};
  ...
  const db = getDb();
  const bestVideo = db.prepare(`
    SELECT v.id, v.title, v.channel_id, ch.niche,
      (SELECT COALESCE(SUM(views),0) FROM metrics WHERE video_id=v.id) views
    FROM videos v JOIN channels ch ON ch.id=v.channel_id
    WHERE v.format='long' ORDER BY views DESC LIMIT 1`).get();
...
export function getAnalyticsChannels(channelId = null) {
  const ids = channelId ? [Number(channelId)] : getDb().prepare("SELECT id FROM channels ORDER BY id").all().map((c) => c.id);
  return ids.map((id) => {
...
export function getAnalyticsNiches(channelId = null) {
  const db = getDb();
  const rows = db.prepare(`WITH
    ...
    GROUP BY c.niche`).all(channelId, channelId);
  return rows.map((r) => {
...
export function getAnalyticsOpportunities(channelId = 1) {
  const channels = getAnalyticsChannels();
  const niches = getAnalyticsNiches();
...
export function getAnalyticsRecommendations(channelId = 1) {
  const channel = getAnalyticsChannels(channelId)[0];
  const content = getAnalyticsContent(channelId);
  ...
  return {
    focus: getWeeklyFocus(channelId),
...
export function getAnalyticsMonetization(channelId = null) {
  const channels = getAnalyticsChannels(channelId);
  const niches = getAnalyticsNiches(channelId);
...
export function getWeeklyFocus(channelId = 1) {
  const channels = getAnalyticsChannels();
...
export function getPriorityRankings(channelId = 1) {
  const channelRows = getAnalyticsChannels();
  ...
  const niches = getAnalyticsNiches().map((n) => ({
    type: "nicho",
    title: n.niche,
    score: n.score.total,
  ...
  const opportunities = getAnalyticsOpportunities(channelId).map((o) => ({
    type: "oportunidade",
    title: o.title,
    score: o.score,
...
export function getAnalyticsExecutive(channelId = 1) {
  const channels = getAnalyticsChannels();
  const niches = getAnalyticsNiches();
  const content = getAnalyticsContent(channelId);
  const opportunities = getAnalyticsOpportunities(channelId);
  const formats = getAnalyticsFormats(channelId);
  const focus = getWeeklyFocus(channelId);
  const rankings = getPriorityRankings(channelId);
  return {
    focus,
...
function saveMediaAssetTx(db, channelId, videoId, asset = {}) {
  db.prepare(`INSERT INTO media_assets
    (channel_id, video_id, asset_type, platform, title, prompt, metadata, status, risk_level)
    VALUES (?,?,?,?,?,?,?,?,?)`).run(
    channelId,
    videoId,
    asset.asset_type,
...
export function getExecutionHistory(limit = 20) {
  return getDb().prepare("SELECT * FROM execution_runs ORDER BY id DESC LIMIT ?").all(limit).map(parseExecutionRun);
}

export function getExecutionStatus() {
  const latest = getDb().prepare("SELECT * FROM execution_runs ORDER BY id DESC LIMIT 1").get();
